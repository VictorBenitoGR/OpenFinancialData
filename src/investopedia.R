...
na.omit(getSymbols(
  c(
    ...
    "V", "TSLA", "WMT", "XOM", "MA",
    "UNH", "PG", "JNJ", "HD", "ORCL", "^GSPC"
  ),
  src = "yahoo",
  periodicity = "monthly",
  from = Sys.Date() - 1858, # ! 1826days = 5years, Added more days this time
  to = Sys.Date(),
  auto.assign = TRUE
))

# Cambiar el nombre de "BRK-B" a "BRK_B"
BRK_B <- get("BRK-B")

list_of_tickers <- list(
  MSFT, AAPL, NVDA, GOOGL, AMZN,
  META, BRK_B, LLY, AVGO, JPM,
  V, TSLA, WMT, XOM, MA,
  UNH, PG, JNJ, HD, ORCL, GSPC
)
...
list_of_tickers <- lapply(sp500$Tickers, function(symbol) {
  data <- na.omit(getSymbols(symbol,
    src = "yahoo",
    periodicity = "daily",
    from = Sys.Date() - 10, # ! 1826days = 5years, Added more days this time
    to = Sys.Date(),
    auto.assign = FALSE
  ))
...
View(portfolio_adjusted)
# PRÓXIMOS 2 FILAS PROMEDIO MÓVIL

library(dplyr)
library(zoo)

# Calcular el promedio móvil para cada columna
portfolio_adjusted_ma <- portfolio_adjusted %>%
  mutate(across(everything(), ~ zoo::rollmean(., k = 3, fill = NA, align = "right")))

# Crear un dataframe con los próximos 11 valores para cada columna
next_11_values <- tail(portfolio_adjusted_ma, 2)

View(next_11_values)

# Obtener la última fila de portfolio_adjusted
last_row_portfolio_adjusted <- tail(portfolio_adjusted, 1)

# Obtener la última fila de next_11_values
last_row_next_11_values <- tail(next_11_values, 1)

# Crear un nuevo dataframe que muestre la diferencia
difference_df <- last_row_next_11_values - last_row_portfolio_adjusted

# Ver el dataframe
View(difference_df)

# Transformar el dataframe a formato largo
difference_df_long <- difference_df %>%
  tidyr::pivot_longer(everything(), names_to = "ticker", values_to = "rendimiento")

# Ver el dataframe
View(difference_df_long)

# Pon el nombre de la empresa tomando como referencia ticker en difference_df_long y Tickers de sp500
difference_df_long <- merge(difference_df_long, sp500, by.x = "ticker", by.y = "Tickers")

empresa <- c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J")
ganancias_totales <- c(105, 96, 58, 23, 101, 135, 54, 78, 89, 92)

df <- data.frame(empresa, ganancias_totales)

# Ordenar el dataframe de mayor a menor ganancias
df <- df[order(-df$ganancias_totales), ]
View(df)

# Calcular el CR4 (microeconomía)
CR4 <- sum(df$ganancias_totales[1:4]) / sum(df$ganancias_totales) * 100
CR4

cuota <- df$ganancias_totales / sum(df$ganancias_totales)
cuota <- data.frame(cuota)
# Calcular la participación de mercado de cada empresa como porcentaje
df$cuota <- df$ganancias_totales / sum(df$ganancias_totales) * 100
View(df)
...
# Hay 3 mercados en una economía. Supongamos que en cada mercado 15 empresas
# cada una con 1% de participación de mercado controlan 15% del mercado
# restante.

# Obtén el CR4 y HHI

# Mercado A: 40%, 20%, 20%, 5%
# Mercado B: 25%, 20%, 20%, 20%
# Mercado C: 75%, 5%, 3%, 2%

# Crear dataframes para cada mercado
mercado_A <- data.frame(cuota = c(40, 20, 20, 5, rep(1, 15)))
mercado_B <- data.frame(cuota = c(25, 20, 20, 20, rep(1, 15)))
mercado_C <- data.frame(cuota = c(75, 5, 3, 2, rep(1, 15)))

# Calcular el CR4 y el HHI para cada mercado
CR4_A <- sum(mercado_A$cuota[1:4])
HHI_A <- sum(mercado_A$cuota^2)

CR4_B <- sum(mercado_B$cuota[1:4])
HHI_B <- sum(mercado_B$cuota^2)

CR4_C <- sum(mercado_C$cuota[1:4])
HHI_C <- sum(mercado_C$cuota^2)
...
write.xlsx(
  ...
  file = "./data/investment_theory2.xlsx"
)
...
returns <- log(portfolio_adjusted / lag(portfolio_adjusted))

# Eliminar las filas con NA
returns <- na.omit(returns)

# Asegurarse de que todas las columnas son numéricas
returns <- data.frame(lapply(returns, as.numeric))

# Crear un objeto de cartera
port <- portfolio.spec(assets = colnames(returns))

# Agregar un objetivo para maximizar el rendimiento esperado
port <- add.objective(portfolio = port, type = "return", name = "mean", target = mean(returns))

# Agregar una restricción para que los pesos de la cartera sumen 1
port <- add.constraint(portfolio = port, type = "full_investment")

# Optimizar la cartera
opt <- optimize.portfolio(R = returns, portfolio = port, optimize_method = "ROI")

# Imprimir los pesos de la cartera óptima
print(opt$weights)
...
conservative_portfolio_beta <- CAPM.beta(conservative_portfolio_returns, GSPC)
conservative_portfolio_treynor <- TreynorRatio(conservative_portfolio_returns, GSPC)
conservative_portfolio_jensen_alpha <- CAPM.jensenAlpha(conservative_portfolio_returns, GSPC)
...
# Import the zoo library
library(zoo)

# Assuming portfolio_adjusted is a dataframe with a single column of data
...
# Apply the function to the portfolio_adjusted dataframe
portfolio_adjusted_ma_n50 <- cols_ma_n50(portfolio_adjusted)

# Forecast the next 11 rows
forecast_values <- rep(mean(tail(portfolio_adjusted_ma_n50$portfolio_adjusted_moving_average_n50, 50)), 11)
...
# Calcular el promedio móvil de 50 días para los días que tienen al menos 49 días anteriores
portfolio_adjusted_ma50 <- rollmean(portfolio_adjusted, k = 50, align = "right")

# Obtener los últimos 11 valores del promedio móvil
predictions <- tail(portfolio_adjusted_ma50, 11)

# Crear un dataframe con las predicciones
predictions_df <- data.frame(
  Date = seq(as.Date(tail(row.names(portfolio_adjusted), 1)) + 1, length.out = 11, by = "day"),
...
portfolio_adjusted_ma50 <- rollmean(portfolio_adjusted, k = 50, align = "right", fill = NA)
